...
sealed class McpServerConfig {
    ...
    /**
     * 本地命令 MCP Server
     * 直接通过 Termux 环境执行本地命令启动 MCP 服务，无需 URL
     * 例如："npx -y @modelcontextprotocol/server-filesystem /sdcard"
     */
    @Serializable
    @SerialName("local_command")
    data class LocalCommandServer(
        override val id: Uuid = Uuid.random(),
        override val commonOptions: McpCommonOptions = McpCommonOptions(),
        val command: String = "",
...
val McpServerConfig.serverUrl: String
    get() = when (this) {
        ...
        is McpServerConfig.LocalCommandServer -> "local://${command}"
    }
