...
class WorkspaceManager(
    ...
    private val environment: WorkspaceEnvironment = WorkspaceEnvironment.PROOT,
) {
     ...
     * 按 [WorkspaceStorageArea.LINUX] 解析依然落空。
     */
    ...
    companion object {
        ...
        /** 创建 Termux 环境专属的 WorkspaceManager */
        fun createTermux(
            baseDir: File,
            nativeLibraryDir: File,
            bindMounts: List<WorkspaceBindMount> = emptyList(),
        ): WorkspaceManager {
            val termuxRunner = ProotShellRunner(
                nativeLibraryDir = nativeLibraryDir,
            )
            val termuxBindMounts = defaultTermuxBindMounts() + bindMounts
            return WorkspaceManager(
                baseDir = baseDir,
                shellRunner = termuxRunner,
                bindMounts = termuxBindMounts,
                environment = WorkspaceEnvironment.TERMUX,
            )
        }

        private fun defaultTermuxBindMounts(): List<WorkspaceBindMount> = listOf(
            WorkspaceBindMount(File("/sdcard"), "/sdcard"),
            WorkspaceBindMount(File("/data/data/com.termux/files/home"), "/data/data/com.termux/files/home"),
            WorkspaceBindMount(File("/data/data/com.termux/files/usr"), "/data/data/com.termux/files/usr"),
        )
    }
}
